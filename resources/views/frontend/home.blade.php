...
<main class="container-fluid px-lg-5 py-4">
            ...
            <div class="glass-card p-4 h-100">
                ...
                <div class="timeline-container position-relative ps-3" style="border-left: 2px dashed var(--nh-border);">
                    ...
                    <div class="timeline-item {{ $loop->last ? '' : 'mb-4' }} position-relative">
                        <span class="position-absolute top-0 start-0 translate-middle p-1 {{ $index === 0 ? 'bg-danger' : 'bg-secondary' }} border border-light rounded-circle" style="left: -13px !important;"></span>
                        <small class="badge bg-secondary text-white mb-1 font-en">{{ $news->created_at->format('h:i A') }}</small>
                        <h4 class="h6 fw-bold mb-1"><a href="{{ route('news.show', $news->slug) }}" class="text-reset text-decoration-none hover-danger">{{ $news->title }}</a></h4>
                        @if($index === 0)
                ...
                <div class="glass-card p-4 h-100">
                    ...
                    <div class="d-flex flex-column gap-3">
                        ...
                        <div class="d-flex gap-3 align-items-start border-bottom pb-3">
                            <span class="fw-black fs-2 text-danger opacity-75 font-en" style="line-height:1;">{{ sprintf('%02d', $index + 1) }}</span>
                            <div>
                                <h4 class="h6 fw-bold mb-1"><a href="{{ route('news.show', $trend->slug) }}" class="text-reset text-decoration-none hover-danger">{{ $trend->title }}</a></h4>
                                <small class="text-muted"><i class="fa-regular fa-eye me-1"></i> {{ number_format($trend->views) }} পড়া হয়েছে</small>
                            </div>
                        </div>
                        @endforeach
                    </div>
            ...
            <div class="col-lg-6">
                <div class="glass-card p-4 h-100">
                    ...
                    <div class="d-flex flex-column gap-3">
                        ...
                        <div class="d-flex gap-3 align-items-start border-bottom pb-3">
                            <span class="fw-black fs-2 text-secondary opacity-50 font-en" style="line-height:1;">{{ sprintf('%02d', $index + 1) }}</span>
                            <div>
                                <h4 class="h6 fw-bold mb-1"><a href="{{ route('news.show', $most->slug) }}" class="text-reset text-decoration-none hover-danger">{{ $most->title }}</a></h4>
                                <small class="text-muted"><i class="fa-regular fa-clock me-1"></i> {{ $most->created_at->diffForHumans() }}</small>
                            </div>
                        </div>
                        @endforeach
                    </div>
    ...
    @php
        ...
        $techCat = \App\Models\Category::where('slug', $techCatSlug)->with(['news' => function ($query) {
    ...
    <section class="p-4 p-md-5 rounded-4 mb-5 position-relative overflow-hidden" style="background: #0B1220; color: #FFFFFF;" data-aos="fade-up">
        <div class="d-flex align-items-center justify-content-between mb-4">
            <h3 class="h4 fw-extrabold text-white border-start border-4 border-danger ps-2 m-0">{{ $techCat->name }}</h3>
        ...
        <div class="row g-4">
            ...
            <div class="col-lg-4">
                <div class="p-3 rounded-3 h-100" style="background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1);">
                    ...
                    <h4 class="h5 fw-bold text-white"><a href="{{ route('news.show', $news->slug) }}" class="text-white text-decoration-none hover-danger">{{ $news->title }}</a></h4>
            ...
            <div class="swiper-wrapper">
                ...
                <div class="swiper-slide">
                    <div class="glass-card overflow-hidden">
                        ...
                        <div class="p-3">
                            <h5 class="h6 fw-bold m-0 line-clamp-2"><a href="{{ route('news.show', $video->slug) }}" class="text-reset text-decoration-none hover-danger">{{ $video->title }}</a></h5>
                        </div>
                    </div>
                </div>
                @endforeach
        ...
        <div class="row align-items-center">
            ...
            <div class="col-lg-5">
                <form action="{{ route('newsletter.subscribe') }}" method="POST" class="d-flex gap-2">
                    @csrf
                    <input type="email" name="email" class="form-control form-control-lg rounded-pill border-0 px-4" placeholder="আপনার ইমেইল লিখুন..." required>
