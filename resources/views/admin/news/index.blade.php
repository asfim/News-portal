            ...
            <form action="{{ route('admin.news.index') }}" method="GET" class="row g-3">
                <div class="col-lg-3 col-md-6">
                    ...
                    <div class="input-group">
                        ...
                        <input type="text" class="form-control bg-light border-0" name="search" value="{{ request('search') }}" placeholder="Search title...">
                    </div>
                </div>
                ...
                <div class="col-lg-2 col-md-12 d-flex align-items-end gap-2">
                    ...
                    @if(request()->anyFilled(['search', 'category_id', 'author_id', 'status']))
                        <a href="{{ route('admin.news.index') }}" class="btn btn-outline-secondary px-3"><i class="fa-solid fa-rotate-left"></i></a>
                    @endif
                </div>
            </form>
        </div>
                                ...
                                <tr class="border-bottom border-light">
                                    <td class="px-4 py-3">
                                        @if($item->featuredImage)
                                            <img src="{{ $item->featuredImage->path }}" alt="{{ $item->title }}" class="rounded-3" style="width: 50px; height: 50px; object-fit: cover;">
                                        @else
                                            <div class="bg-light text-secondary rounded-3 d-flex align-items-center justify-content-center" style="width: 50px; height: 50px;">
                                    ...
                                    <td class="py-3" style="max-width: 280px;">
                                        ...
                                        <span class="small text-secondary">{{ $item->created_at->format('M d, Y H:i') }}</span>
                                    </td>
                                    <td class="py-3">
                                        <span class="badge bg-secondary bg-opacity-10 text-secondary">{{ $item->category->name }}</span>
                                    </td>
                                    <td class="py-3 text-secondary small">{{ $item->author->name }}</td>
                                    <td class="py-3">
                                        <div class="d-flex flex-wrap gap-1">
                                            @if($item->breaking_news)
                                                <span class="badge bg-danger py-1 px-2" style="font-size: 0.7rem;">Breaking</span>
                                            @endif
                                            @if($item->featured_news)
                                                <span class="badge bg-primary py-1 px-2" style="font-size: 0.7rem;">Featured</span>
                                            @endif
                                            @if($item->trending_news)
                                                <span class="badge bg-warning text-dark py-1 px-2" style="font-size: 0.7rem;">Trending</span>
                                            @endif
                                            @if($item->editor_choice)
                                                <span class="badge bg-info text-dark py-1 px-2" style="font-size: 0.7rem;">Editor Choice</span>
                                            @endif
                                    ...
                                    <td class="py-3">
                                        @switch($item->status)
                                            ...
                                            @case('draft')
                                                ...
                                                @break
                                            @case('scheduled')
                                                <span class="badge bg-primary bg-opacity-10 text-primary rounded-pill px-3 py-2">Scheduled</span>
                                                @break
                                            @default
                                                <span class="badge bg-secondary bg-opacity-10 text-secondary rounded-pill px-3 py-2">{{ ucfirst($item->status) }}</span>
                                        @endswitch
                                    </td>
                                    <td class="py-3 text-center fw-semibold text-secondary small">{{ number_format($item->views) }}</td>
                                    <td class="px-4 py-3 text-end">
                                        <a href="{{ route('admin.news.edit', $item->id) }}" class="btn btn-light btn-sm text-secondary border-0"><i class="fa-regular fa-pen-to-square"></i></a>
