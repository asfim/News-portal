            ...
            <form action="{{ route('admin.contacts.index') }}" method="GET" class="row g-3">
                <div class="col-lg-6 col-md-8">
                    ...
                    <div class="input-group">
                        ...
                        <input type="text" class="form-control bg-light border-0" name="search" value="{{ request('search') }}" placeholder="Search by sender name, email or subject...">
                    </div>
                </div>
                ...
                <div class="col-lg-3 col-md-12 d-flex align-items-end gap-2">
                    ...
                    @if(request()->anyFilled(['search', 'status']))
                        <a href="{{ route('admin.contacts.index') }}" class="btn btn-outline-secondary px-3"><i class="fa-solid fa-rotate-left"></i></a>
                    @endif
                </div>
            </form>
        </div>
                    ...
                    <tbody>
                        ...
                        @else
                            @foreach($contacts as $msg)
                                <tr class="border-bottom border-light {{ $msg->status === 'unread' ? 'fw-bold bg-light bg-opacity-50' : '' }}" id="msg-row-{{ $msg->id }}">
                                    <td class="px-4 py-3">
                                        @if($msg->status === 'unread')
                                            <span class="badge bg-danger rounded-circle p-1.5"><span class="visually-hidden">New</span></span>
                                        @else
                                    ...
                                    <td class="py-3 text-dark text-truncate" style="max-width: 250px;">{{ $msg->subject }}</td>
                                    <td class="py-3 text-secondary small">{{ $msg->created_at->format('M d, Y H:i') }}</td>
                                    <td class="px-4 py-3 text-end">
                                        <div class="d-inline-flex gap-1">
                                            <button type="button" class="btn btn-light btn-sm text-primary border-0 inspect-btn" 
                                                    data-id="{{ $msg->id }}"
                                                    data-name="{{ $msg->name }}" 
                                            ...
                                            <button type="button" class="btn btn-light btn-sm text-secondary border-0 toggle-read-btn" data-id="{{ $msg->id }}" title="Toggle read/unread"><i class="fa-regular fa-envelope"></i></button>
...
<div class="modal fade" id="messageInspectModal" tabindex="-1" aria-labelledby="messageInspectModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content rounded-4 border-0 shadow-lg">
            <div class="modal-header px-4 py-3 bg-light">
                <h5 class="modal-title fw-bold text-dark" id="messageInspectModalLabel"><i class="fa-regular fa-envelope me-1"></i> Read Inquiry</h5>
            ...
            <div class="modal-body p-4">
                ...
                <div class="mb-4 bg-light p-4 rounded-3 border">
                    ...
                    <p class="text-dark mb-0" id="inspectMessage" style="white-space: pre-wrap; line-height: 1.6;"></p>
                </div>
            </div>
            <div class="modal-footer px-4 py-3 bg-light text-end">
                <button type="button" class="btn btn-secondary px-4 py-2 rounded-pill fw-semibold" data-bs-dismiss="modal">Close</button>
            </div>
        </div>
    </div>
</div>
